...
class Desktop(BaseSystem):
	...
	def __init__(self, gamesdb):
		BaseSystem.__init__(self, gamesdb, "desktop")
		self.path = self.gamesdb.path
	...
	def update_db(self):
		...
		for entry in self.get_game_desktop_entries():
			id = None
			for row in db.execute('SELECT id FROM ' + self.system + ' WHERE path = ?', [entry]):
				id = row[0]
			
			# Search in the games table
			if not id:
				db.execute('INSERT INTO ' + self.system + ' (id, path) VALUES (NULL, ?)', [entry])
				for row in db.execute('SELECT id FROM ' + self.system + ' WHERE path = ?', [entry]):
					id = row[0]
